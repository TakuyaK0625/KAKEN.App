...
DF <- bind_rows(d2018, d2019) %>% 
    ...
    # 審査区分コード
    mutate(区分コード = ifelse(str_detect(審査区分, "中区分|小区分"), str_extract(審査区分, "\\d+?(?=:)"), "")) %>%
    mutate(区分コード = ifelse(str_detect(審査区分, "大区分"), str_replace(審査区分, "大区分", ""), 区分コード)) %>%
    ...
    select(-`研究期間 (年度)`, -期間, -研究機関, -研究代表者, -審査区分)

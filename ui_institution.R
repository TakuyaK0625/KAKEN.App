...
# UI
tabItem_institution <- tabItem(tabName = "Radar",
                               sidebarLayout(
                                  ...
                                  tabsetPanel(type = "tabs",
                                              ...
                                              tabPanel("（注）図の見方", 
                                                       ...
                                                       p("このレーダーチャートは、各機関の審査区分ごとの採択件数や直接経費の総額を一覧するための
                                                         グラフです。値は常用対数に変換して補正した値と百分率を用いていますが、それぞれの詳細は以下の通りです。"),
                                                       br(),
                                                       p("【対数】最上位の機関の値は4であり、最小値は0。1の変化は実際の値の10倍、1/10に相当し, 0.5の差は実際の値の3.3倍, 1/3.3に相当する。"),
                                                       p("（参考）https://www.ruconsortium.jp/uploaded/life/248_387_misc.pdf"),
                                                       br(),
                                                       p("【百分率】下から数えて全体の何%の位置にあるのかを表す。0〜100の値をとるが、グラフでは50〜100の範囲のみ表示。")
